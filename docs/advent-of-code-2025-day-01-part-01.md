...
Is thinking that the dial starts by pointing at 50.
...
As a result I've reduced the dial from `0..99` to `0..9` by taking everything modulo 10:
...
This works only if the puzzle cares exclusively about the last digit.

However, a dial numbered 0–99 is normally a 100-position circular dial, not a 10-position dial. The correct state transition is:

$new\:position=\left(position\pm movement\right)\%\:100$

not modulo 10.

$\left(50−68\right)mod100=82$
